...
export default function FixedSizeGrid(props) {
  ...
  const columns = [
    ...
    { field: 'conversion', headerName: 'Fare' , width: 250,valueGetter: (params) => params.row.conversion.PHP
  ...
  return (
    <div style={{ width: "80%", margin: "50px auto", background: "white" }}>
      <div style={{ height: 350, width: "100%" }}>
        <DataGrid 
        getRowId={(row) =>row.booking_token+""+row.utc_arrival+""+(new Date().getTime())
         }
        rows={rows}
        columns={columns}
        pageSize={5}
